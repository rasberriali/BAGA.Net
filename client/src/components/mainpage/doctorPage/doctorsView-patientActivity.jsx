...
import Update from './doctors-update';
import View from './doctors-view';
  ...
  const handleSubmit = async () => {
    if (files.length > 0 && !isSubmitting) {
      ...
      try {
        ...
        setIsModalOpen(false); // Optionally close the modal after submission
      } catch (error) {
        console.error('Error uploading files:', error.response ? error.response.data : error.message);
        alert('Error uploading files. Please try again!');
      } finally {
        setIsSubmitting(false);  // Reset submitting state once the process is complete
      }
    }
  };
            ...
            <table className="table-auto border-collapse w-full">
              <thead className="bg-gray-200 sticky top-0 z-10">
                <tr className="bg-gray-200 text-left text-gray-700">
                  ...
                  <th className="px-6 py-2 text-start">Xray</th> {/* New column for X-ray */}
